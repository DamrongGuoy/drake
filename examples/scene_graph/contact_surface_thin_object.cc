/** @file
 A simple binary for exercising and visualizing computation of
 ContactSurfaces of thin objects. This is decoupled from dynamics so that just
 the geometric components can be evaluated in as light-weight a fashion as
 possible.

 It serves as a test bed for evaluating ContactSurface-computing algorithms
 that involve thin objects.  */
...
DEFINE_double(simulation_time, 60.0,
              ...
              "By default, it is 60 seconds, which is the time for the "
              "moving soft ball to start midway between the thin plate and "
              "the bowl, go up to the plate, go down to the bowl, and come "
              "back to the starting point.");
DEFINE_double(real_time, 1.0, "Real time factor.");
DEFINE_double(length, 0.0125,
...
DEFINE_bool(plate, true, "true = use the thin palte, false = do not use "
                         "the thin plate. Default true.");
DEFINE_bool(bowl, true, "true = use the bowl, false = do not use the bowl. "
                        "Default true.");

/** Places a ball and defines its velocity as being sinusoidal in time in the
 z direction.

 The moving soft ball starts from a point O in the empty space between
 the thin plate and the bowl.
   1. It goes up to the plate.
   2. It goes down to O.
   3. It goes down to the bowl.
   4. It finally goes up to O again.
...
class MovingBall final : public LeafSystem<double> {
 public:
  ...
  static constexpr double kT = 60.0;
  // 2π/T
  static constexpr double k2Pi_T = 2.0 * M_PI / kT;
  // z(t) ∈ [5cm, 13cm], so Zo = 9cm and A = 4cm.
  // Initial position, Zo = 9cm above ground.
  static constexpr double kZo = 0.09;
  // Amplitude of motion, A = 4 cm.
  static constexpr double kA = 0.04;
...
int do_main() {
  ...
  // Thin rigid anchored plate with frame P.
  if (FLAGS_plate) {
    // Place the plate 14cm above the ground plane.
    const double height = 0.14;
    const RigidTransformd X_WP(Vector3d(0, 0, height));
    // The plate is 5cm x 5cm x 2mm.
    const double length = 0.05;
    const double thickness = 0.002;
    GeometryId thin_box_id = scene_graph.RegisterAnchoredGeometry(
        source_id,
        make_unique<GeometryInstance>(
            X_WP, make_unique<Box>(length, length, thickness), "rigid plate"));
    ProximityProperties rigid_props;
    // Coarsest tessellation.
    AddRigidHydroelasticProperties(length, &rigid_props);
    scene_graph.AssignRole(source_id, thin_box_id, rigid_props);
    IllustrationProperties illus_props;
    illus_props.AddProperty("phong", "diffuse", Vector4d{0.4, 0.5, 0.45, 0.25});
    scene_graph.AssignRole(source_id, thin_box_id, illus_props);
  }

  // Rigid anchored bowl with frame B. It is a non-convex mesh.
  if (FLAGS_bowl) {
    std::string bowl_absolute_path =
        FindResourceOrThrow("drake/examples/scene_graph/evo_bowl_no_mtl.obj");
    ...
    ProximityProperties rigid_props;
    AddRigidHydroelasticProperties(1. /*unused*/, &rigid_props);
