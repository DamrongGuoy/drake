...
namespace internal {
namespace {
void WriteObj(std::ofstream& out, const TriangleSurfaceMesh<double>& mesh) {
  // Example:
  // "v  1.0 -1.0 -1.0\n"
  // "v  1.0 -1.0  1.0\n"
  // "v -1.0 -1.0  1.0\n"
  ...
  for (int v = 0; v < num_vertices; ++v) {
    const Vector3<double>& vertex = mesh.vertex(v);
  ...
  // Example:
  // "f 1 2 3\n"}
  const int num_faces = mesh.num_triangles();
  for (int f = 0; f < num_faces; ++f) {
    const SurfaceTriangle& face = mesh.element(f);
