  ...
  vtkUnstructuredGridQuadricDecimationEdge& finalE, vtkUnstructuredGridQuadricDecimationQEF& minQ)
{
  ...
  for (int j = 0; j < 2; j++)
  {
    for (int i = 0; i < setSize; i++)
    {
      ...
      if ((e.Verts[0]->Q.p[3] < 0 && e.Verts[1]->Q.p[3] > 0) ||
          (e.Verts[0]->Q.p[3] > 0 && e.Verts[1]->Q.p[3] < 0)) {
        // Skip the edge with two vertices of opposite signs of field values.
        i = i - 1;
        continue;
      }
...
int vtkUnstructuredGridQuadricDecimationTetMesh::Simplify(int n, int desiredTets)
{
  ...
  while ((count < n || desiredTets < (tCount - unusedTets)) && (run < 1234567))
  {
