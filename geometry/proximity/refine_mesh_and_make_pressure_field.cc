...
int do_main(int argc, char* argv[]) {
  ...
  VolumeMeshFieldLinear<double, double> pressure_field =
      internal::MakeVolumeMeshPressureField(&refined_mesh,
                                            /* hydroelastic_modulus*/ 1e7,
                                            /* margin*/ 0);
  std::filesystem::path outfile_field(std::string(argv[2]) + "_pressure.vtk");
  internal::WriteVolumeMeshFieldLinearToVtk(outfile_field.string(), "pressure",
                                            pressure_field,
