...
class BVH {
 ...
 private:
  ...
  // Optimizes a given oriented bounding box `box0` of a point set to decrease
  // its volume.
  static Obb OptimizeObbVolume(
      const MeshType& mesh_M,
      const std::set<typename MeshType::VertexIndex>& vertices,
      const Obb& box0);

  // Calculates the gradient of the volume of an oriented bounding box as a
  // function of roll-pitch-yaw angle.
  static Vector3<double> CalcVolumeGradient(
      const MeshType& mesh_M,
      const std::set<typename MeshType::VertexIndex>& vertices,
      const Obb& box);

  static Obb CalcOrientedBox(
      const MeshType& mesh_M,
      const std::set<typename MeshType::VertexIndex>& vertices,
      const math::RotationMatrixd& X_MF);
