...
GTEST_TEST(TestComputeOBB, SingleTri) {
  ...
  const Obb obb = BVHTester::ComputeBoundingVolume<SurfaceMesh<double>>(
      mesh_M, tri_centroids.begin(), tri_centroids.end());
  ...
  EXPECT_TRUE(
      CompareMatrices(obb.pose().matrix(), expect_pose.matrix(), 1e-15));
...
TEST_F(BVHTest, TestComputeBoundingVolume) {
  {
    ...
    Vector3d Bx_M = Vector3d::UnitZ();
    Vector3d By_M = Vector3d::UnitY();
    Vector3d Bz_M = -Vector3d::UnitX();
    RigidTransformd expect_X_MB(
        RotationMatrixd::MakeFromOrthonormalColumns(Bx_M, By_M, Bz_M),
        Vector3d::Zero());

    EXPECT_TRUE(CompareMatrices(obb.pose().matrix(), expect_X_MB.matrix()));
    EXPECT_TRUE(CompareMatrices(obb.half_width(), Vector3d(1.5, 1.5, 1.5),
                                ObbTester::kTolerance));
  }
  ...
  // half width of 1, 2, 3.
  {
    ...
    EXPECT_TRUE(CompareMatrices(obb.pose().matrix(), expect_X_MB.matrix()));
    EXPECT_TRUE(CompareMatrices(obb.half_width(), Vector3d(3., 2., 1.),
                                ObbTester::kTolerance));
  }
}
...
TEST_F(BVHTest, TestBuildBVTree) {
  WriteBVHToVtk("octahedron_obb_bvh.vtk", bvh_,
                "OBB Tree of SurfaceMesh of Octahedron");
  WriteSurfaceMeshToVtk("octahedron_surface_mesh.vtk", mesh_,
...
TEST_F(BVHTest, TestCollideSurfaceVolume) {
  ...
  WriteBVHToVtk("elliposid_tetrahedra_obb_bvh.vtk", tet_bvh,
