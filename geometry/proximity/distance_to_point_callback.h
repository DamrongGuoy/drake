...
/* The callback function for computing the signed distance between a point and
 ...
 @param[in,out] callback_data  A CallbackData parameter, whose const members
                               are input, and the `CallbackData::distances`
                               is the output.

 @param[out] threshold  An output parameter (pass-by-reference) to work
                        with FCL's broadphase culling. It will be set
                        to the value of CallbackData<T>::threshold or an
                        internal lower bound related to the machine epsilon.
