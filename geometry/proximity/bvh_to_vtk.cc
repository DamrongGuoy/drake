...
template <typename MeshType>
int GetAllNodes(const BVHNode<MeshType>& node, int depth,
                std::vector<const BVHNode<MeshType>*>* all_nodes,
                std::vector<int>* all_depths,
                std::vector<int>* all_heights) {
  all_nodes->push_back(&node);
  all_depths->push_back(depth);
...
void WriteVtkUnstructuredGrid(
    std::ofstream& out,
    const std::vector<const BVHNode<MeshType>*>& bvh_nodes) {
  std::vector<Vector3d> vertices;
  for (const BVHNode<MeshType>* node : bvh_nodes) {
    const Obb& box_B = node->obb();
