...
class QueryObject {
  ...
  */
  ...
  /** Reports pairwise intersections and characterizes each non-empty
  intersection as the volume of intersection between overlapping geometries.
  Each contact volume is represented by the boundary surface that encloses
  the volume.

  For a given overlapping volume, hereinafter we denote its domain with Ω
  and its boundary with ∂Ω. For each of these volumes, the boundary can be
  split into two sub-domains ∂Ωₘ and ∂Ωₙ, each associated with one of the
  original geometries M and N respectively. ∂Ωₘ is included in the boundary
  of geometry M and ∂Ωₙ is included in the boundary of geometry N. ∂Ωₘ and
  ∂Ωₙ are disjoint (∂Ωₘ ∩ ∂Ωₙ = ∅), and their union is the full boundary ∂Ω
  of the overlapping volume.

  The ContactSurface refers to a linear field that at each point x ∈ ∂Ω
  provides the signed distance from x to N, when x ∈ ∂Ωₘ, or from x to M,
  when x ∈ ∂Ωₙ.

  @note In the current implementation, only contacts between geometry::Mesh
   (or geometry::Convex) with compliant-hydroelastic properties are supported,
   i.e, the geometry::Mesh must come from VTK tetrahedral meshes
   (geometry::Convex can come from both Obj surface meshes and VTK
   tetrahedral meshes).   Other kinds of geometry::Shape, e.g., Box, Capsule,
   Sphere, are ignored. Other geometry::Mesh with rigid-hydroelastic
   properties or non-hydroelastic properties are ignored.  We will extend
   the implementation to cover all kinds of geometry::Shape in the future.

  @note As an improvement to hydroelastics, the tetrahedral meshes do not need
   interior vertices. There is no need to refine the interior volume of
   the mesh.  A tetrahedral mesh from a very coarse Constrained Delauany
   Tetrahedrization without interior vertices are ideal for performance reasons,
   e.g., a coarse mesh generated by TetGen without refinement.

  @note A another improvement to hydroelastics, contact surfaces ∂Ωₘ and ∂Ωₙ
   are available even in challenging situations like very deep penetration, one
   object completely enclosing another object, or in the presence of thin
   objects. There is no filtering of face normals along field gradients.
